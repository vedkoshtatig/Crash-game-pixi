...
private hydrationResolver!: ()=>void;
private hydrationPromise = new Promise<void>(res=>{
  this.hydrationResolver = res;
});
private api: ApiClient
  ...
  // step 3 wait first realtime tick
  await this.waitFirstRealtimeState()
  onProgress?.(1)

  this.hydrated = true
}
  }
  public async waitForInitialSync(onProgress?: (p:number)=>void) {

  // simulate multiple async
  await this.loadHistory();
  onProgress?.(0.4);

  await this.fetchCurrentRoundState();
  onProgress?.(0.7);

  await this.fetchMyBetState();
  onProgress?.(1);

  this.hydrated = true;
  this.hydrationResolver();
}
