...
if (!isMobile) {
  ...
  const historyWidth = rightWidth - GAP * 5;
  const historyHeight = 50;
 const betWidth = leftWidth*1.1;
  this.betHistory.resize(historyWidth, historyHeight);

  this.betHistory.position.set(
    betWidth + GAP * 2, // ⭐ position relative to BET PANEL (not fly area)
    10
  );
  ...
  this.betPanel.resize(betWidth, panelHeight);
