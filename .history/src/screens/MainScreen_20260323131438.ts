...
export class MainScreen extends Container {
  ...
  private async init() {
    ...
    this.addChild(this.betPanel, this.betHistory, this.flyArea);
  }
  ...
  private layout() {
    ...
    if (!isMobile) {
      ...
      this.betHistory.position.set(leftWidth + GAP * 2, GAP);

      this.betHistory.resize(rightWidth, betHistoryHeight);
      ...
      this.flyArea.resize(rightWidth, flyHeight);
    } else {
      // ⭐⭐⭐⭐⭐ MOBILE LAYOUT

      // BET HISTORY (TOP FULL WIDTH)
      this.betHistory.position.set(GAP, GAP);
      this.betHistory.resize(width - GAP * 2, betHistoryHeight);

      // FLY AREA (MIDDLE)
      const flyHeight = height * 0.52;

      this.flyArea.position.set(GAP, betHistoryHeight + GAP * 2);

      this.flyArea.resize(width - GAP * 2, flyHeight);

      // BET PANEL (BOTTOM FULL WIDTH)
      const panelHeight = height - betHistoryHeight - flyHeight - GAP * 4;

      this.betPanel.position.set(GAP, betHistoryHeight + flyHeight + GAP * 3);

      this.betPanel.resize(width - GAP * 2, panelHeight);
    }
  }
}
