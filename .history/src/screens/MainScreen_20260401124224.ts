...
if (!isMobile) {
  ...
  this.flyArea.position.set(leftWidth + GAP * 2, 0);
  this.flyArea.resize(rightWidth, height);
  ...
  const panelHeight = height * 0.38; // 👈 instead of full height

  this.betPanel.resize(leftWidth, panelHeight);

  this.betPanel.position.set(
    GAP,
    height - panelHeight - 20 // 👈 bottom floating
  );
} else {
