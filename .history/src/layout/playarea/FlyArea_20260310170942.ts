...
createClouds() {
  ...
  for (let i = 0; i < 12; i++) { // more clouds
    ...
    // random position across entire sky
    cloud.x = Math.random() * this.flyWidth;
    cloud.y = Math.random() * this.flyHeight;

    // speed based on size (parallax)
    (cloud as any).speed = scale * 2;
...
updateClouds() {
  ...
  for (let cloud of this.clouds) {
    ...
    // if cloud exits screen → reset to top-right
