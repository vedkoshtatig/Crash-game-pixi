...
this.isCrashed = false;
 ...
 flyPlane(time: number, multiplier: number) {
  ...
  const runwayTime = 0.25; // takeoff earlier

  const x = Math.min(
    this.startX + time * 550,   // faster horizontal speed
    maxX
  );
  ...
  if (time < runwayTime) {
    ...
    y = Math.max(
      this.startY - Math.pow(flyTime, 2) * 110,   // stronger lift
      maxY
    );
  ...
  if (time > runwayTime) {
    ...
    const targetRotation = Math.min(flyTime * 0.22, 0.45); // rotate faster
