...
this.plane.scale.set(planeBaseScale * 0.085);
  ...
  initEvents() {
    gameEvents.on("round:waiting", (data: { seconds: number }) => {
      ...
      SoundManager.instance.play("takeOff", false);
      // SoundManager.instance.play("timer", false);
    ...
    gameEvents.on("round:start", () => {
      SoundManager.instance.stop("blast");
  ...
  flyPlane(time: number) {
    ...
    SoundManager.instance.play("takeOff", false, 0.4 , () => {
      SoundManager.instance.play("fly", true, 0.4);
    });
...
SoundManager.instance.play("timer");
  ...
  crashPlane(rate: number) {
    ...
    SoundManager.instance.play("blast" , false);
...
this.blast.scale.set(baseScale * 0.2);
