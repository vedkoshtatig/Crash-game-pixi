...
private skyAnimating = false;
  ...
  init() {
    ...
    this.world = new Container();0x001a66
    this.addChild(this.world);
this.skyOverlay = new Graphics();
this.skyOverlay
  .clear()
  .rect(0, 0, this.WORLD_WIDTH, this.WORLD_HEIGHT)
  .fill(color);

this.skyOverlay.alpha = 0;

this.world.addChild(this.skyOverlay);
    this.world.addChild(
  ...
  if (this.skyState.t < 0.5) {
    color = this.lerpColor(start, mid, this.skyState.t * 2);
  } else {
    color = this.lerpColor(mid, end, (this.skyState.t - 0.5) * 2);
  }

  this.skyOverlay.clear();
  this.skyOverlay.beginFill(color);
  this.skyOverlay.drawRect(0, 0, this.WORLD_WIDTH, this.WORLD_HEIGHT);
