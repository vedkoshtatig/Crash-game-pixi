...
private cloudTexture = Texture.from("/plane-idle.png");
...
createClouds() {
  ...
  const cols = 5;   // horizontal spread
  const rows = 3;   // vertical spread
  const padding = 100;

  const cellW = this.flyWidth / cols;
  const cellH = this.flyHeight / rows;
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      // place inside grid cell with small randomness
      cloud.x = c * cellW + cellW / 2 + (Math.random() - 0.5) * padding;
      cloud.y = r * cellH + cellH / 2 + (Math.random() - 0.5) * padding;

      (cloud as any).speed = 0.5 + scale * 1.5;
...
updateClouds() {
  ...
  for (let cloud of this.clouds) {
    ...
    // if cloud exits screen → respawn randomly on right side
    if (cloud.x < -150 || cloud.y > this.flyHeight + 150) {

      cloud.x = this.flyWidth + Math.random() * 200;
      cloud.y = Math.random() * this.flyHeight * 0.6;
...
if (!this.zoomTriggered && x >= this.flyWidth * 0.25) {
  ...
  if (time < runwayTime) {
    ...
    y = Math.max(
      this.startY - Math.pow(flyTime, 2) * 100,
      maxY
    );
