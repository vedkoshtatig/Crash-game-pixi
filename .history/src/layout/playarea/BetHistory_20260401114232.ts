...
export class BetHistory extends Container {
  ...
  // 🖤 1. DARK BASE (more solid)
  .roundRect(0, 0, width, height, 20)
  .fill({ color: 0x0b1220, alpha: 0.92 }) // deep bluish-black glass

  // ✨ 2. SOFT INNER LIGHT (glass feel)


  // 🌟 3. TOP SHINE STRIP (subtle reflection)
  .roundRect(2, 2, width - 4, height * 0.35, 18)
  .fill({ color: 0xffffff, alpha: 0.08 })

  // 💎 4. OUTER BORDER SHINE
  .roundRect(0, 0, width, height, 20)
  .stroke({
    color: 0xffffff,
    alpha: 0.18,
  ...
  // 🖤 5. INNER DARK EDGE (depth)
  .roundRect(1, 1, width - 2, height - 2, 20)
  .stroke({
    color: 0x000000,
    alpha: 0.35,
    width: 1
