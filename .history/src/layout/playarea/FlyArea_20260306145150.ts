...
layout() {
  ...
  this.startY = this.flyAreaHeight - 60;

  this.graph.clear();
  this.graph.lineStyle(4, 0xff2f55); // SET ONCE
...
drawGraph(multiplier: number) {
  ...
  const x = this.startX + this.time * 80;
  const y = this.startY - Math.pow(multiplier, 1.4) * 20;

  this.graph
    .moveTo(this.lastX, this.lastY)
    .lineTo(x, y)
    .stroke({ width: 4, color: 0xff2f55 });
