      ...
      if (s.phase === "WAITING") {
        if (this.uiAutoMode) {
          ...
          if (s.hasBet) {
            ...
            amt.textContent = s.betAmount.toFixed(2) + " USD";
            btn.onclick = () => {
              s.setAutoBet(false); // IMPORTANT → kill auto engine
              s.placeBet();
            };
          }
        }
      ...
      //  FLYING
        ...
        if (s.hasBet && !s.hasCashedOut) {
          ...
          if (s.scheduledBet) {
            ...
            title.textContent = this.uiAutoMode ? "Auto Bet" : "Bet";
            amt.textContent = s.betAmount.toFixed(2) + " USD";
            btn.onclick = () => {
              if (this.uiAutoMode) {
                s.setAutoBet(true);
...
`;
    ...
    quickBtns.forEach((q) => {
      ...
      const toggle = card.querySelector(".bc-toggle") as HTMLElement;
      const autoInput = card.querySelector(
        ".bc-autocash-val"
      ) as HTMLInputElement;
  ...
  layout(width?: number, height?: number) {
    ...
    this.bg.clear().roundRect(0, 0, width, height, 20).fill(0x1a1a1a);
  }
