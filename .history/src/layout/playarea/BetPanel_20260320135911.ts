...
autoInput.value = s.autoCashoutMultiplier.toFixed(2);
...
if (s.hasBet && !s.hasCashedOut) {
  autoInput.style.pointerEvents = "none"
  autoInput.style.opacity = "0.5"
} else {
  autoInput.style.pointerEvents = "auto"
  autoInput.style.opacity = "1"
}
      //  WAITING
      if (s.phase === "WAITING") {
        if (s.autoBetEnabled) {
