...
this.bg.y += speed * 2.9 * this.sceneScale * ease;
  ...
  this.plane.scale.set(0.069 * this.sceneScale);
    ...
    const sceneScale = Math.min(this.flyWidth, this.flyHeight) / 900;
this.sceneScale = sceneScale;
  ...
  flyPlane(time: number) {
    ...
    const x = Math.min(this.startX + time * 600 * this.sceneScale, maxX);
  ...
  crashPlane(rate: number) {
    ...
    this.blast.scale.set(0.1 * this.sceneScale);
