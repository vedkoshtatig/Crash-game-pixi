...
private flightPhase: "WAITING" | "RUNNING" | "TAKEOFF" | "FLYING" = "WAITING";
  private bg!: Sprite;
  ...
  this.flightPhase = "WAITING";   // ⭐ phase set
...
flyPlane(time: number) {
  ...
  const runwayTime = 0.45;
  const takeoffRotateEnd = 1.2;        // ⭐ time when rotation finishes
  const cruiseX = this.flyWidth - 350;
  const cruiseY = 420;

  // ⭐ PHASE TRANSITIONS
  if (this.flightPhase === "RUNNING" && time >= runwayTime) {
    this.flightPhase = "TAKEOFF";
  }

  if (this.flightPhase === "TAKEOFF" && time >= takeoffRotateEnd) {
    this.flightPhase = "FLYING";
  }

  // =========================
  // ⭐ WAITING PHASE (safety)
  // =========================
...
// ⭐ TAKEOFF → FLYING (POSITION BASED SMOOTH)
if (
  this.flightPhase === "TAKEOFF" &&
  this.plane.x >= cruiseX - 2 &&
  this.plane.y <= cruiseY + 2
) {
  this.flightPhase = "FLYING";
}

  // =========================
  // ⭐ TAKEOFF (rotation + lift)
  // =========================
  if (this.flightPhase === "TAKEOFF") {

  const flyTime = time - runwayTime;

  const progress = Math.min(flyTime / 1.4, 1);   // ⭐ normalized takeoff progress

  const x =
    this.startX +
    (cruiseX - this.startX) * progress;

  const liftEase = 1 - Math.pow(1 - progress, 2.2);   // ⭐ smooth cinematic ease

  const y =
    this.startY +
    (cruiseY - this.startY) * liftEase;
  ...
  // =========================
  // ⭐ FLYING (stable cruise)
  // =========================
  if (this.flightPhase === "FLYING") {
  ...
  this.plane.x = cruiseX;
  this.plane.y = cruiseY + hover;
