...
export class FlyArea extends Container {
  ...
  init() {
   ...
   this.plane.scale.set(0.069 * this.sceneScale);
  ...
  update() {
    if (this.bgDrift) {
      ...
      this.bg.x -= speed * 2.3 * this.sceneScale;
      ...
      if (this.serverTime > runwayTime) {
        ...
        const ease = t * t * t;
this.bg.y += speed * 2.9 * this.sceneScale * ease;
  ...
  layout() {
    ...
    this.startY = this.flyHeight * 0.67;
    }else{
       this.startX = this.flyWidth * 0.15;
  ...
  this.plane.scale.set(0.069 * this.sceneScale);
    ...
    const sceneScale = Math.min(this.flyWidth, this.flyHeight) / 550;
this.sceneScale = sceneScale;
  ...
  flyPlane(time: number) {
    ...
    let speedX
 if(this.flyWidth>=920){
    const x = Math.min(this.startX + time * 300 * this.sceneScale, maxX);
    }else{
       const x = Math.min(this.startX + time * 560 * this.sceneScale, maxX);
    }
     const x = Math.min(this.startX + time * speedX * this.sceneScale, maxX);
  ...
  crashPlane(rate: number) {
    ...
    this.blast.scale.set(0.3 * this.sceneScale);
