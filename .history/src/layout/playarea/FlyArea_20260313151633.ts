...
flyPlane(time: number) {
  ...
  // ⭐ PHASE detection based on visual progress
  const progress = (x - this.startX) / (maxX - this.startX);

  let newPhase: "RUNNING" | "TAKEOFF" | "FLYING";

  if (progress < 0.30) newPhase = "RUNNING";
  else if (progress < 0.55) newPhase = "TAKEOFF";
  ...
  // ⭐ vertical motion (smooth thrust lift)
  let y;
