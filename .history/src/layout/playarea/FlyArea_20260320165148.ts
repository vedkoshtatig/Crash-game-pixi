...
export class FlyArea extends Container {
  ...
  update() {
    ...
    if (!this.takeoffPlayed && this.serverTime >= runwayTime) {
      ...
      this.plane.state.addAnimation(0, "Fly", true, 0);
    }
  ...
  layout() {
    ...
    // ⭐ responsive plane scale
let planeScale;

if (this.flyWidth <= 920) {
  // mobile tighter framing
  planeScale = this.flyWidth / 18000;
} else {
  // desktop cinematic
  planeScale = this.flyWidth / 14000;
}
...
this.plane.scale.set(planeScale);
    ...
    // cover style scaling
 // ⭐ cinematic camera zoom based on screen size
const baseScaleX = this.flyWidth / this.bg.texture.width;
const baseScaleY = this.flyHeight / this.bg.texture.height;

let cameraZoom;

if (this.flyWidth <= 920) {
  // mobile → closer camera feel
  cameraZoom = 1.25;
} else {
...
const finalScale = Math.max(baseScaleX, baseScaleY) * cameraZoom;
...
this.bg.scale.set(finalScale);
  ...
  flyPlane(time: number) {
    ...
    const x = Math.min(this.startX + time * 420, maxX);
    ...
    const cruiseY = this.flyHeight * 0.36;
