...
public resize(w: number, h: number) {
  ...
  updateClouds() {
    for (let cloud of this.clouds) {
      ...
      let speed;
      speed = baseSpeed + this.multiplier * 2;
  ...
  // ⭐ ONLY fly if store says flying
  if (phase === "FLYING") {
    this.flyPlane(this.store.multiplier);
  }
}
 ...
 initEvents() {
  ...
  this.store.subscribe(() => {
    ...
    if (phase === "WAITING") {
      ...
      this.waitTimer(5); // you can later sync exact seconds
    ...
    if (phase === "CRASHED") {
      this.crashPlane(this.store.crashPoint);
    }
 ...
 flyPlane(multiplier: number){
  const time = Math.log2(multiplier) / 0.09;
    this.multiplierText.visible = true;
