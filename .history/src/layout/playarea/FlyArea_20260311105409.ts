...
const runwaySpeed = 900;   // increase this for faster runway
const flySpeed = 480;      // keep flight speed same

let x;

if (time < runwayTime) {
  // runway acceleration
  x = this.startX + time * runwaySpeed;
} else {
  const flyTime = time - runwayTime;
  x = this.startX + runwayTime * runwaySpeed + flyTime * flySpeed;
}
