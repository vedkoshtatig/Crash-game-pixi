...
// smoothly approach server time
const diff = this.targetServerTime - this.serverTime;
this.serverTime += diff * 0.1;

// continue normal time flow
this.serverTime += dt;
...
this.isCrashed = false;
    ...
    gameEvents.on(
      ...
      (data: { time: number; multiplier: number }) => {
       ...
       this.targetServerTime = data.time;
