...
// 🔴 1. DARK SOLID BASE
pill.roundRect(0, 0, pill.width, pill.height, r)
  .fill({ color: baseColor || color, alpha: 0.95 });

// ✨ 2. INNER GLASS GLOW (soft light)
pill.roundRect(0, 0, pill.width, pill.height, r)
  .fill({ color: 0xffffff, alpha: 0.06 });

// 🌟 3. TOP SHINE (glass reflection strip)
pill.roundRect(2, 2, pill.width - 4, pill.height * 0.45, r)
  .fill({ color: 0xffffff, alpha: 0.12 });

// 💎 4. BORDER SHINE (KEY PART YOU WANTED)
pill.roundRect(0, 0, pill.width, pill.height, r)
  .stroke({
    color: 0xffffff,
    alpha: 0.25,
...
// 🖤 5. SUBTLE INNER DARK EDGE (depth)
pill.roundRect(1, 1, pill.width - 2, pill.height - 2, r)
  .stroke({
    color: 0x000000,
    alpha: 0.25,
    width: 1
