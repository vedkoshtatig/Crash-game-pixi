...
import { showCashoutPopup } from "../../ui/CashoutPopup";
...
s._popupShown = false;
  if (s.autoBetEnabled) {
    ...
    // ⭐ if bet already placed this round → show CANCEL
    if (s.hasBet) {
    ...
    // ⭐ bet not yet placed → show AUTO BET
    else {
  ...
  // ⭐ USER ALREADY CASHED THIS ROUND
 if (s.hasCashedOut) {

  // 🔥 ADD THIS GUARD (IMPORTANT)
  if (!s._popupShown) {

    const multiplier = s.multiplier;
    const amount = s.winAmount > 0 
      ? s.winAmount 
      : (s.betAmount * s.autoCashoutMultiplier);

    showCashoutPopup(multiplier, amount);

    s._popupShown = true; // prevent spam (VERY IMPORTANT)
  }
...
<div class="bc-autocash-row">
  ...
  <div class="bc-autocash-left">
    ...
    <div class="bc-autocash-input">
      <input class="bc-autocash-val" value="2" />
...
this.wrapper.style.justifyContent = "flex-start"; // 🔥 stick to top
this.wrapper.style.alignItems = "stretch";        // full width
}
...
//   .fill({ color: 0xffffff, alpha: 1 }); // very subtle base
}
