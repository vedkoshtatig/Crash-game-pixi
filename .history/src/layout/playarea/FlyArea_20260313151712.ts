...
flyPlane(time: number) {
  ...
  this.plane.texture = this.runTexture;   // ⭐ keep OLD behaviour
  ...
  // ⭐ derive phase from motion progress
  const progress = (x - this.startX) / (maxX - this.startX);

  let newPhase: "RUNNING" | "TAKEOFF" | "FLYING";

  if (progress < 0.28) newPhase = "RUNNING";
  else if (progress < 0.50) newPhase = "TAKEOFF";
  else newPhase = "FLYING";

  // ⭐ ONLY DEBUG PRINT (no animation change)
  if (newPhase !== this.flightPhase) {
    this.flightPhase = newPhase;
    console.log("✈️ PHASE →", newPhase);
  }
  ...
  if (time < runwayTime) {
    ...
    const lift =
      (1 - Math.exp(-flyTime * 2.2)) * (this.startY - cruiseY);

    const climbY = this.startY - lift;
  ...
  if (time > runwayTime) {
    const flyTime = time - runwayTime;
    const targetRotation = Math.min(flyTime * 0.5, 0.45);
