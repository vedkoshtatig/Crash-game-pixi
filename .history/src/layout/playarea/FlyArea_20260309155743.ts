...
update() {
  ...
  let multiplier = Math.pow(2, time * 0.09);

  // prevent overshoot
  multiplier = Math.min(multiplier, this.serverMultiplier);
...
gameEvents.on("plane:update", (data: { time: number }) => {
  ...
  // store server multiplier
  this.serverMultiplier = Math.pow(2, data.time * 0.09);
...
console.log(multiplier);
  ...
  const y = Math.max(this.startY - Math.pow(t, 1.7) * 90, maxY);
