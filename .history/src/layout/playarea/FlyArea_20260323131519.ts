...
private worldProgressY = 0;
  ...
  constructor() {
    ...
    app.ticker.add(this.update, this);window.addEventListener("resize", this.layout);
  }
...
this.plane.scale.set(planeScale);
  ...
  public resize(w: number, h: number) {
    ...
    if (this.flyWidth > 0 && this.flyHeight > 0) {
      ...
      this.bg.position.set(0, this.flyHeight);
    }
...
this.plane.scale.set(planeScale);
  ...
  layout() {
    ...
    this.startY = this.flyHeight * 0.7;
       ...
       this.startX = this.flyWidth * 0.15;
    this.startY = this.flyHeight * 0.72;
    }
