...
this.plane.scale.set(planeBaseScale * 0.085);
  ...
  // ⭐ ALWAYS update clouds if active
if (this.cloudsActive) {
  this.updateClouds();
}

// ⭐ THEN handle flying logic
if (!this.isFlying) return;
  ...
  // ⭐ ALWAYS sync time if flying
  if (this.isFlying) {
    this.serverTime += dt;
  }

  // ⭐ FORCE CLOUD STATE BASED ON TIME (CRITICAL FIX)
  if (this.serverTime > 0.55) {
    if (!this.cloudsActive) {
      this.activateClouds(); // create helper
    }
  }
