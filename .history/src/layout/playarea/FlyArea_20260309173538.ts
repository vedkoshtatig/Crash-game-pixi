...
private targetX = this.startX;
private targetY = this.startY;
  private flyWidth = 0;
  private flyHeight = 0;
  ...
  constructor() {
    ...
    app.ticker.add(this.animatePlane, this);
  }
...
this.timerText = new Text({
 ...
 flyPlane(time: number, multiplier: number) {
  ...
  this.targetX = Math.min(this.startX + t * 300, maxX);
  this.targetY = Math.max(this.startY - Math.pow(t, 1.7) * 180, maxY);
...
animatePlane() {
  ...
  const smooth = 0.08; // smaller = smoother

  this.plane.x += (this.targetX - this.plane.x) * smooth;
  this.plane.y += (this.targetY - this.plane.y) * smooth;
