...
document.addEventListener("visibilitychange", () => {
  if (!document.hidden) {
    ...
    // ⭐ FORCE CLOUD RECOVERY
    if (this.cloudsActive) {
      this.clouds.forEach((cloud) => {
        // put clouds INSIDE screen immediately
  ...
  init() {
    ...
    const planeBaseScale = Math.min(this.flyWidth, this.flyHeight) / 1050;
this.plane.scale.set(planeBaseScale * 0.085);
    ...
    this.clouds.forEach(cloud => this.addChild(cloud));
  }
 ...
 update() {
  ...
  // ⭐ keep serverTime in sync ONLY if flying
  if (this.isFlying) {
    this.serverTime += dt;
  }

  // ⭐ ALWAYS derive cloud state from time (NOT from animation)
  if (this.serverTime > 0.55 && !this.cloudsActive) {
    this.activateClouds();
  }

  // ⭐ ALWAYS update clouds (independent of flying)
  if (this.cloudsActive) {
    this.updateClouds();
  }

  // ⭐ background drift (can run even if not flying for smoothness)
  if (this.bgDrift) {
    const mult = CrashGameStore.instance.multiplier;
  ...
  if (this.skyAnimating) {
    ...
    let color;
    const t = 0; // since DARKEN_SPEED = 0
...
this.plane.scale.set(planeBaseScale * 0.085);
  ...
  // ⭐ FORCE SYNC VISUAL STATE
 if (this.serverTime > 0.55) {
  if (!this.cloudsActive) {
    this.cloudsActive = true;

    this.clouds.forEach((cloud) => {
      // ⭐ spawn INSIDE instead of outside
      cloud.x = Math.random() * this.flyWidth;
      cloud.y = Math.random() * this.flyHeight;
...
const x = Math.min(this.startX + time * speed, maxX);
   ...
   const shouldHaveClouds = time > runwayTime + 0.15;

if (shouldHaveClouds && !this.cloudsActive) {
  this.cloudsActive = true;
  ...
  this.clouds.forEach((cloud) => {
    ...
    const cruiseY = this.flyHeight * 0.25;
