...
this.skyOverlay
  ...
  // ⭐ target darkness based on multiplier
  const targetT = Math.min(multiplier / 8, 1);

  // ⭐ smooth approach (THIS IS THE MAGIC)
  this.skyState.t += (targetT - this.skyState.t) * 0.05;
 ...
 resetPlane() {
  ...
  gsap.to(this.plane, {
    ...
    onComplete: () => {
      ...
      // ⭐ NOW sky resets AFTER plane reaches runway
      this.skyAnimating = false;
      this.skyState.t = 0;
  ...
  duration: 1.2,
  ease: "power2.out",
  onStart: () => this.skyAnimating = true,
  onComplete: () => this.skyAnimating = false
});
    }
  });
}
