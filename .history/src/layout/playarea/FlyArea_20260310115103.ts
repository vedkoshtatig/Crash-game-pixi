...
private flightPhase: "idle" | "runway" | "takeoff" | "fly" = "idle";
  ...
  update() {
   ...
   if (!this.isFlying || this.flightPhase !== "fly") return;
...
startRunway() {
  ...
  this.flightPhase = "runway";
  this.plane.texture = this.runTexture;

  const runwayTime = 1.5;
  const startX = this.startX;
  const endX = this.startX + 120;
