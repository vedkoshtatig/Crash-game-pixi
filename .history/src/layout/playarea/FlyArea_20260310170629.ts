...
createClouds() {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    cloud.scale.set(0.4 + Math.random() * 0.4);

    // spawn outside top-left
    cloud.x = this.flyWidth + 200 + Math.random() * 200;
    cloud.y = -200 - Math.random() * 200;

    this.clouds.push(cloud);
    this.addChildAt(cloud, 1); // behind plane
  }
}
updateClouds() {
  ...
  for (let cloud of this.clouds) {
    ...
    // if cloud leaves screen → reset to top-left
    if (cloud.x > this.flyWidth + 200 || cloud.y > this.flyHeight + 200) {

      cloud.x = -200 - Math.random() * 300;
      cloud.y = -200 - Math.random() * 200;
