...
public resize(w: number, h: number) {
  ...
  this.viewportMask.width = w / scale;
  this.viewportMask.height = h / scale;
  ...
  return {
    left: 0,
    right: this.viewWidth / this.scale.x,
    top: 0,
    bottom: this.viewHeight / this.scale.y,
  };
}
  ...
  flyPlane(time: number, multiplier: number) {
    ...
    if (time > runwayTime) {
      ...
      if (reachedCruise) {
        ...
        this.plane.rotation =
          -0.45 + Math.sin(t) * 0.04 + Math.sin(t * 3) * 0.015;
      } else {
