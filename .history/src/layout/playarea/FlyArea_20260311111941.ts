...
// first entry speed
if ((cloud as any).initialBoost) {
  speed = 15;

  // after first frame switch to normal logic
  (cloud as any).initialBoost = false;

} else {
  speed = baseSpeed + this.multiplier/1.8 ;
}
...
if (cloud.x < -200 || cloud.y > this.flyHeight + 200) {
  ...
  // when respawning give boost again
  (cloud as any).initialBoost = true;
 ...
 resetScene() {
  ...
  flyPlane(time: number, multiplier: number) {
    ...
    if (time < runwayTime) {
      ...
      y = Math.max(this.startY - Math.pow(flyTime, 2) * 300, maxY);
    }
