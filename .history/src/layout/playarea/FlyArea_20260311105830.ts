...
export class FlyArea extends Container {
  ...
  for (let i = 0; i < cloudCount; i++) {
    ...
    (cloud as any).speed = 0.3 + scale * 2.2;
...
this.multiplierText.anchor.set(0.5)
 ...
 resetScene() {
  ...
  fflyPlane(time: number, multiplier: number) {
...
const runwayAccel = 1600; // runway acceleration
const flightAccel = 900;  // flight acceleration

let x;

if (time < runwayTime) {
// accelerating on runway
x = this.startX + runwayAccel * time * time;
} else {
const flyTime = time - runwayTime;


const runwayDistance = runwayAccel * runwayTime * runwayTime;

// faster acceleration after takeoff
x = this.startX + runwayDistance + flightAccel * flyTime * flyTime;
...
// stronger upward acceleration
y = Math.max(this.startY - 220 * flyTime * flyTime, maxY);
...
if (time > runwayTime) {
const flyTime = time - runwayTime;
const targetRotation = Math.min(flyTime * 0.45, 0.45);
