...
`;
  ...
  // ⭐ allow click ONLY during waiting
  if (!this.isWaiting && !(this.isFlying && this.hasBet[index])) return;

  // ⭐ WAITING → schedule bet
  if (this.isWaiting) {
    ...
    // ⭐ disable till flight starts
    btn.style.pointerEvents = "none";
    return;
  }

  // ⭐ CASHOUT during flight
  if (this.isFlying && this.hasBet[index]) {
    console.log("CASHED OUT");

    this.hasBet[index] = false;

    this.setButtonState(card, "disabled");
  ...
  public setButtonState(
    card: HTMLElement,
    state: "normal" | "schedule" | "cashout" | "disabled",
    multiplier?: number
  ) {
