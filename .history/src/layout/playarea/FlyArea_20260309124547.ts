...
export class FlyArea extends Container {
  ...
  private maxrotation: number = -0.5;
  private isRunning = false;
  private runTime = 0;
  private runDuration =1; // seconds
  private isTakingOff = false;
private takeoffTime = 0;
private takeoffDuration = 0.4;
  ...
  update() {
    if (this.isRunning) {
  this.plane.scale.set(0.25)
  this.runTime += app.ticker.deltaTime / 60;

  // runway speed matches flight speed
  this.plane.x += this.speed * 520;

  // vibration
 this.plane.y = this.startY - 20;

  if (this.runTime >= this.runDuration) {
...
this.startX = this.plane.x;
this.startY = this.plane.y;

    this.t = 0;

    this.startX = this.plane.x;
    this.startY = this.plane.y;
    this.plane.rotation = -this.runTime * 0.15;
  }
}
if (this.isTakingOff) {

  this.takeoffTime += app.ticker.deltaTime / 60;

  const progress = this.takeoffTime / this.takeoffDuration;

  // smooth forward motion
  this.plane.x += this.speed * 520 * 0.7;

  // gentle lift
  this.plane.y = this.startY - progress * 40;

  // smooth tilt
  this.plane.rotation = -progress * 0.4;
  ...
  if (progress >= 1) {
    ...
    if (this.isFlying) {
      this.plane.scale.set(0.25);
      if (this.plane.rotation > this.maxrotation) {
        this.plane.rotation -= 0.0015;
      ...
      const y = this.startY - Math.pow(this.t, 2) * 120 - 20;
