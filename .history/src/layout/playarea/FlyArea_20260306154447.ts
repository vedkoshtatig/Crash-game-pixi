...
this.graphY = y;
  ...
  // small wave motion
  const yOffset = Math.sin(this.hoverTime) * 8;

  // move graph slowly forward
  this.graphX += 0.5;

  const newY = this.graphY + yOffset;

  this.trail
    .lineTo(this.graphX, newY)
    .stroke({
      width: 4,
      color: 0xff3b3b,
      cap: "round",
  ...
  this.plane.position.set(this.graphX, newY);
}
