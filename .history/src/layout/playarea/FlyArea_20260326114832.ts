...
this.plane.scale.set(planeBaseScale * 0.085);
  ...
  update() {
    ...
    this.serverTime += dt;
    const runwayTime = 0.42;
 const liftThreshold = this.startY - this.flyHeight * 0.02; 
// ⭐ small lift detection (tweak value)

if (!this.takeoffPlayed && this.plane.y < liftThreshold) {
