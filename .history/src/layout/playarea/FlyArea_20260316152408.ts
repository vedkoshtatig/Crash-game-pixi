...
private bg!: Graphics;
private bgColor = { value: 0x87ceeb }; // light blue
...
gsap.to(state, {
  ...
  onUpdate: () => {
    ...
    if (state.t < 0.5) {
      color = lerpColor(start, mid, state.t * 2);
    } else {
      color = lerpColor(mid, end, (state.t - 0.5) * 2);
    }
...
this.world.addChildAt(this.bg, 0);
  ...
  update() {
    this.bg.tint = this.bgColor.value;
...
this.bg.clear();
this.bg.beginFill(this.bgColor.value);
this.bg.drawRect(0, 0, this.WORLD_WIDTH, this.WORLD_HEIGHT);
this.bg.endFill();
