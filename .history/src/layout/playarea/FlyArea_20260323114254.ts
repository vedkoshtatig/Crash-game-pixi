...
export class FlyArea extends Container {
  ...
  createClouds() {
    ...
    const cloudCount = Math.floor(area / 90000) + 6;
    // mobile ~12
    // desktop ~18-24
    // ultra wide ~30+
    ...
    if (this.bgDrift) {
      ...
      if (this.serverTime > runwayTime) {
        ...
        this.bg.y += speed * 2.9 * ease;
          const takeoffProgress = Math.min((this.serverTime - runwayTime) * 0.6, 1);

// start scale
const baseScale = 0.05;

// max scale when cruising
const maxScale = 0.12;

// smooth easing
const scale =
  baseScale + (maxScale - baseScale) * takeoffProgress;

this.plane.scale.set(scale);
      }
    }
  ...
  flyPlane(time: number) {
    ...
    const maxX = this.flyWidth - this.flyWidth * 0.32;

    const x = Math.min(this.startX + time * 430, maxX);
    ...
    const cruiseY = this.flyHeight * 0.32;
