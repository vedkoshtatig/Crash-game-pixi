...
export class FlyArea extends Container {
  ...
  private flyWidth = 0;
  private flyHeight = 0;
private planeProgressX = 0;   // 0 → 1 across width
private planeProgressY = 0;   // 0 → 1 across height
  private startX = 0;
  private startY = 0;
  ...
  const oldW = this.flyWidth || width;
  const oldH = this.flyHeight || height;
  ...
  // ⭐ preserve camera offset ratio
  const camRatioX = this.bg.x / oldW;
  const camRatioY = this.bg.y / oldH;

  this.bg.x = camRatioX * width;
  this.bg.y = camRatioY * height;

  // Plane start reference
  this.startX = width * 0.1;
  this.startY = height * 0.86;

  // ⭐ keep plane where it was visually
  if (this.isFlying || this.isCrashed) {
    this.plane.x = this.planeProgressX * width;
    this.plane.y = this.planeProgressY * height;
  } else {
  ...
  this.multiplierText.position.set(width / 2, height / 2);

  // Timer
  this.timerText.position.set(width / 2, 200);

  // ⭐ clouds maintain relative position
  for (let cloud of this.clouds) {
    const rx = cloud.x / oldW;
    const ry = cloud.y / oldH;
  ...
  flyPlane(time: number, multiplier: number) {
    ...
    this.planeProgressX = this.plane.x / this.flyWidth;
this.planeProgressY = this.plane.y / this.flyHeight;
  }
