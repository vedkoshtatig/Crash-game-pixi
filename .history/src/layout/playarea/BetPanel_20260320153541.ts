...
private isAutoBet : boolean = false;
  ...
  private updateFromStore() {
    ...
    cards.forEach((card) => {
      const uiMode = card.getAttribute("data-ui-mode") === "auto"
      ...
      //  FLYING
        ...
        if (s.hasBet && !s.hasCashedOut) {
          ...
          if (s.scheduledBet) {
            ...
            title.textContent = uiMode? "Auto Bet" : "Bet";
            amt.textContent = s.betAmount.toFixed(2) + " USD";
            btn.onclick = () => s.scheduleBet();
          }
        }
...
card.style.flexShrink = "0";
 ...
 card.setAttribute("data-ui-mode", "bet")
...
autoTab.onclick = () => {
  ...
  card.setAttribute("data-ui-mode", "auto")
