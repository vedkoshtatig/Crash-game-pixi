  ...
  cards.forEach((card) => {
    ...
    if (s.phase === "WAITING") {
      ...
      if (s.hasBet) {
        ...
        title.textContent = this.isAutoBet ? "Auto Bet" : "Bet";
        amt.textContent = s.betAmount.toFixed(2) + " USD";

        btn.onclick = () => {

          if (s.autoBetEnabled) {
            s.scheduleBet();     // ⭐ auto mode → schedule
          } else {
            s.placeBet();        // ⭐ normal mode → instant
          }
    ...
    // ⭐ CRASHED
      ...
      if (s.scheduledBet) {
        ...
        title.textContent = isAutoBet ? "Auto Bet" : "Bet";
        amt.textContent = s.betAmount.toFixed(2) + " USD";
    ...
    // ⭐ LOCK AMOUNT WHEN ACTIVE BET
    const amountControls = card.querySelector(".bc-middle-row") as HTMLElement;

    const shouldLockAmount =
      s.scheduledBet ||
      (s.hasBet && !s.hasCashedOut);

    amountControls.style.opacity = shouldLockAmount ? "0.4" : "1";
    amountControls.style.pointerEvents = shouldLockAmount ? "none" : "auto";
