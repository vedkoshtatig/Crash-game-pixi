...
export class FlyArea extends Container {
  ...
  private isTakeoffAnimating = false;
...
this.isCrashed = false;
  ...
  // FIRST CALL → TAKEOFF
 if (!this.takeoffPlayed) {

  this.takeoffPlayed = true;
  this.isTakeoffAnimating = true;

  gsap.timeline({
    onComplete: () => {
      this.isTakeoffAnimating = false;
    }
  })
...
if (this.isTakeoffAnimating) {
  return;
}
  // NORMAL FLYING (smooth updates)
console.log("takeoff end");
  gsap.to(this.plane, {
    x,
    y,
    duration: 0.2,
