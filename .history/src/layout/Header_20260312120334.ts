import { Graphics, Text, Container } from "pixi.js";
import { app } from "../main";
import { CrashGameStore } from "../store/GameStore";
export class Header extends Container {

  private bg!: Graphics;
  private logo!: Text;
private store = CrashGameStore.instance;
  private balance!: Container;
  public amount!: Text;
  private currency!: Text;

  constructor() {
    super();

    this.init();
    this.layout();
this.store.subscribe(() => this.updateFromStore());
    window.addEventListener("resize", () => this.layout());
  }

  init() {

    // background
    this.bg = new Graphics();

    // logo
    this.logo = new Text({
  text: "SkyRush",
  style: {
    fontFamily: "Star Shield",
    fontSize: 40,
    fill: "#0db6ee",
    fontWeight: "normal",
    letterSpacing: 2
  }
});
    // balance container
    this.balance = new Container();

    this.amount = new Text({
      text: this.store.balance,
      style: {
    fill: "#ffffff",
        fontWeight: "700"
      }
    });

    this.currency = new Text({
      text: "USD",
      style:{
        fill: "#ffffff",
        fontWeight: "700"
      }
    });

    this.balance.addChild(this.amount, this.currency);

    this.addChild(this.bg, this.logo, this.balance);
   
  }
private updateFromStore() {
  this.amount.text = this.store.balance.toFixed(2);

  // currency position changes because width changes
  this.currency.position.set(this.amount.width + 6, 0);

  this.layout(); // optional but good for right alignment
}
 layout() {

  const { width, height } = app.screen;
  const headerHeight = height / 18;

  this.bg.clear()
    .rect(0, 0, width, headerHeight)
   .fill(0x121821);

  this.logo.position.set(20, headerHeight / 2 - this.logo.height / 2);

  this.amount.position.set(0, 0);
  this.currency.position.set(this.amount.width + 6, 0);

  this.balance.position.set(
    width - this.balance.width - 20,
    headerHeight / 2 - this.balance.height / 2
  );
}
}