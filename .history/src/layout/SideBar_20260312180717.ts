...
export class SideBar extends Container {
  ...
  layout() {
    ...
    // ⭐ outer pill
    this.tabsBg.clear()
      .roundRect(0,0, this.sidebarWidth - 30, this.panelHeight/11, 40)
      .fill(0x121821);
      this.tabsBg.pivot.set(this.tabsBg.width/2,this.tabsBg.height/2)
this.tabsBg.position.set(this.sidebarWidth / 2,this.panelHeight/17)
    // ⭐ active capsule
    this.selectedTab.clear()
      .roundRect(
        0,
        0,
        this.tabsBg.width-100,
        this.tabsBg.height-20,
        26
      )
      .fill(0x2b3038);
      this.selectedTab.pivot.set(this.selectedTab.width/2,this.selectedTab.height/2);
      this.selectedTab.position.set(this.sidebarWidth/2,this.panelHeight/17)

    // ⭐ centered text
    this.titleMyBets.anchor.set(0.5)
    this.titleMyBets.x =
      this.sidebarWidth / 2 ;
      this.titleMyBets.width=this.selectedTab.width-160;
    if(this.titleMyBets.width<=this.selectedTab.height-20){
this.titleMyBets.height=this.selectedTab.height-20;
    } 
    this.titleMyBets.y = this.panelHeight/17;
    this.titleMyBets.width=this.selectedTab.width-160;
    if(this.titleMyBets.height<=this.selectedTab.height-20){
this.titleMyBets.height=this.selectedTab.height-20;
    } 
    ...
    this.roundTitle.y = this.panelHeight/6.5;
