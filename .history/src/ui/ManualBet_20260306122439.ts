import { Container, Graphics } from "pixi.js";

export class ManualBet extends Container {

  public bg: Graphics;
  public selectorBg : Graphics;
  public bet : Graphics;
  public auto : Graphics;
  public betController : Graphics;
  public betAmount : Text;
  public predefinedBet : Graphics;
    

  constructor() {
    super();

    this.bg = new Graphics();
    this.selectorBg = new Graphics();
    this.bet = new Graphics()
    this.auto = new Graphics()
    this.betController = new Graphics()
    this.betAmount = new Text; 
    this.predefinedBet = new Graphics()
    
    this.selectorBg.addChild(this.bet,this.auto)

    this.addChild(this.bg,this.selectorBg,this.betController);
  }

  layout(width: number, height: number) {

    this.bg.clear()
      .roundRect(0, 0, width, height, 20)
      .fill(0x222222);

    this.selectorBg.clear()
    .roundRect(0,0,215,30,15)
    .fill(0xff000)
    .pivot.set(107.5,0)
    this.selectorBg.position.set(width/2 ,8)
     
    this.bet.clear()
    .roundRect(5,3,100,24,15)
    .fill(0xff0000)
    this.auto.clear()
    .roundRect(108,3,100,24,15)
    .fill(0xff0000)

    this.betController.clear()
    .roundRect(100,10,100,10,10)
    .fill(0xff0000)
    
    

  }
}